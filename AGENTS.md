...
1. **数据含义**：引用字段/指标前确认自己理解其含义，不要想当然
2. **机制解释**：不查代码就不说代码怎么工作。"可能是"开头的机制描述不该出现在 issue 或报告里
3. **测试覆盖**：改了代码就在真实路径验证（lifecycle script 在干净环境，API 在实际调用），不是只跑 happy path
4. **审计方法**：每条纠正标注用的命令和 commit hash，结论可复现

"大概是这样"不是验证。查代码、跑命令、看输出——然后才能下结论。

这条从 beliefs-candidates.md 升级而来（5 次重复：03-23 数据含义 ×2、04-02 审计方法、04-04 不查代码编造机制、04-06 安装路径未测试）。
