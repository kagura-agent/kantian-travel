...
- 环境:Node 24(v24.16.0), Python 3.12.3 (Go: not installed), gh CLI, Claude Code | OpenClaw 2026.6.6 (8c802aa) | Memory vector search ❌ 不可用（06-13 三轮 eval: 0% 成功率。根因: openai-compatible embedding provider 已从 config 移除，当前 provider 全是 anthropic-messages 无 embedding 能力。索引存在 13,045 chunks 但无法查询。需重新配置 embedding provider 或 `openclaw memory index --force`）。memory_get 100% 可靠
