...
- 2026-03-24: [gradient] "要学习人家优秀的地方 还有就是我们可以通过学习去学习一些优秀的人" → [行为改变] 打工不只是修bug，是学习维护者的工程思维。study 步骤加“读维护者最近 merged PR”，从对比中学习 (pattern: learn-from-maintainers, 第1次)
...
- 2026-03-28: [gradient] Luna: "修复时要知道为什么错了，人家方案为什么更好" → [行为改变] review 修复不只是改代码，要写清楚根因分析和正确模式。这是学习机会不是修 bug (pattern: learn-from-maintainers, 第2次)
...
- 2026-04-30: [gradient] "Luna: 搞不定就先退,别引起反感" + "不能解决就及时止损礼貌退出,不纠缠" → [行为改变] 规范严格的项目(如 kilocode),先观察 PR 模式和规范再动手。被 maintainer 多次提醒说明准备不足,应该主动关 PR + 礼貌收尾 + 退出观察,而不是继续追问。纠缠只会消耗对方耐心、损害信誉。(pattern: oss-retreat, 第1次)
  - source: human
  - triggers: PR 被 maintainer 连续纠正 2+ 次时
  - validation: 是否在被纠正后主动关闭 PR 而非继续追问
