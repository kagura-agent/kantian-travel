...
- **可复用教训**（下次遇到类似情况该怎么做）→ 用 add-gradient.sh 写入：
  `bash ~/.openclaw/workspace/tools/add-gradient.sh "教训内容" "行为改变" --source nudge`
- **项目/领域知识、灵感火花** → wiki/（按 wiki/README.md 规范写）
⚠️ 不要全塞 memory——memory 是流水账，沉底后不会被复用
**自主学习也算**：cron/study/workloop 中发现了更好的做法、踩了坑、工具 bug → 同样走 add-gradient.sh
没犯也没学到 → 跳过
...
- 没有重复 → **用 add-gradient.sh 写入**：
  `bash ~/.openclaw/workspace/tools/add-gradient.sh "反馈内容" "行为改变" --source nudge`
...
- **用 add-gradient.sh 写入**：
  `bash ~/.openclaw/workspace/tools/add-gradient.sh "做了什么" "为什么值得记" --source nudge`
